...
final class TodoItemTests: XCTestCase {
    ...
    func testTodoItemToCSVWithAllParameters() {
        ...
        XCTAssertEqual(components[6], dateFormatter.string(from: modificationDate))
    }
    ...
    func testTodoItemToCSVWithComma() {
        ...
        let creationDate = Date().addingTimeInterval(-86400)
        let modificationDate = Date()
        let item = TodoItem(id: "12345", text: "Test, with comma", priority: .important, deadline: deadline, isCompleted: true, creationDate: creationDate, modificationDate: modificationDate)
        let csvString = item.csv
        let expectedCSV = "12345,\"Test, with comma\",important,\(dateFormatter.string(from: deadline)),true,\(dateFormatter.string(from: creationDate)),\(dateFormatter.string(from: modificationDate))"
    ...
    func testTodoItemFromCSVWithComma() {
        ...
        let deadlineString = dateFormatter.string(from: Date().addingTimeInterval(86400))
        let modificationDateString = dateFormatter.string(from: Date())
        let csvString = "12345,\"Test, with comma\",important,\(deadlineString),true,\(creationDateString),\(modificationDateString)"
